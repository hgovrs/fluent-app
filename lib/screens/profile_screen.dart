  ...
  Future<void> _authenticate() async {
    ...
    try {
      ...
      if (success) {
        _password.clear();
        _message(
          'Conta conectada. Você escolhe quando salvar seu progresso na nuvem.',
        );
      }
    } finally {
  ...
  Future<void> _sync({required bool restore}) async {
    final confirmed = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        title: Text(
          restore ? 'Trazer progresso da nuvem?' : 'Salvar nesta conta?',
        ),
        content: Text(
          restore
    ...
    try {
      if (restore) {
        ...
        final success = await widget.cloud.backup(
          widget.controller.exportProgress(),
        );
        if (success) _message('Backup salvo na sua conta.');
      }
    } catch (_) {
      _message(
        'Não foi possível recuperar este backup. Seu progresso local foi mantido.',
      );
    } finally {
      if (mounted) setState(() => _working = false);
    }
      ...
      builder: (context, _) {
        ...
        return Column(
          ...
          children: [
            ...
            const Text(
              'Pequenos hábitos, grandes descobertas.',
            ...
            SurfaceCard(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 8),
                  const Text(
                    'Escolha sua meta diária. Você pode mudar quando quiser.',
                    style: kSecondaryTextStyle,
                  ),
                    ...
                    children: [
                      for (final goal in [10, 20, 40])
                        ChoiceChip(
                          ...
                          labelStyle: progress.dailyGoal == goal
                              ? kSelectedChipTextStyle
                              : kChipTextStyle,
                          onSelected: (_) =>
                              widget.controller.setDailyGoal(goal),
                        ),
                    ],
                  ),
                  const SizedBox(height: 20),
                  DropdownButtonFormField<String>(
                    ...
                    decoration: const InputDecoration(
                      labelText: 'Trilha em prática',
                    ),
                    isExpanded: true,
                    dropdownColor: kOverlaySurface,
                    borderRadius: kCardRadius,
                    elevation: 0,
                    iconEnabledColor: kBrandPurpleLight,
                    items: [
                  ...
                  const Text(
                    'Trilhas de prática parcial A1–C1. Concluir exercícios não certifica proficiência.',
                    style: kSecondaryTextStyle,
                  ),
                ],
              ),
            ...
            SurfaceCard(
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  const Text(
                    'As lições funcionam sem conta e sem internet. Uma conta é opcional para guardar um backup e levá-lo a outro aparelho.',
                    style: kSecondaryTextStyle,
                  ),
                  const SizedBox(height: 16),
                  if (!cloud.available)
                    const SurfaceCard(
                      ...
                      child: Text(
                        'Modo local ativo. O Firebase ainda não foi configurado nesta versão. Seu progresso fica salvo neste dispositivo.',
                        style: kSecondaryTextStyle,
                      ),
                    )
                  else if (cloud.signedIn) ...[
                    Text(
                      cloud.email ?? 'Conta conectada',
                    ...
                    TextButton(
                      onPressed: busy
                          ...
                          : () async {
                              ...
                              if (!cloud.signedIn) {
                                _message(
                                  'Conta desconectada. O progresso local continua neste aparelho.',
                                );
                              }
                            },
                    ...
                    Form(
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          const Text(
                            'Ao criar uma conta, seu e-mail é enviado ao Firebase para autenticação. Apenas o backup que você solicitar será enviado à nuvem.',
                            style: kSecondaryTextStyle,
                          ),
                        ],
                      ),
                  ...
                  if (busy) ...[
                    ...
                    const SizedBox(
                      height: 64,
                      child: AppLoading(
                        semanticsLabel: 'Conectando ao Firebase',
                      ),
                    ),
                  ],
                  if (cloud.error != null) ...[
            ...
            const SizedBox(height: 24),
            const Text(
              'Feito para aprender, não para competir.',
              style: TextStyle(fontWeight: FontWeight.w700),
            ),
            const SizedBox(height: 6),
            const Text(
              'Sem anúncios, assinatura, ranking ou limite de vidas. Seus dados locais permanecem neste aparelho ao sair da conta; limpar os dados do app os remove.',
              style: kSecondaryTextStyle,
            ),
          ],
        );
  ...
  Widget _stat(BuildContext context, String value, String label) => Column(
    ...
    children: [
      Text(
        ...
        style: Theme.of(
          context,
        ).textTheme.headlineMedium?.copyWith(color: kBrandPurpleLight),
      ),
